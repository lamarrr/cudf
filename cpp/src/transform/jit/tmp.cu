...
struct null_mask_chunk {
  ...
  // TODO: document
  // TODO: document requirements, i.e. output must be aligned, size checks, etc.
  // similar to cudf::bitmask_... but allows inlining of operands
  /// @brief
  /// @warning num_chunks and src_size must be >= 0
  template <typename BitMaskWordFunc>
  static __device__ null_mask_chunk load(cudf::size_type chunk,
                                         BitMaskWordFunc&& src_null_mask,
                                         cudf::size_type src_offset,
                                         cudf::size_type src_size)
  {
    static_assert(
      sizeof(cudf::bitmask_type) == 4,
      "bitmask_type must be 32 bits");  // important checks for __funnelshift_r and __popc
