 ...
 */
[[nodiscard]] std::unique_ptr<cudf::column> calculate_amount(cudf::column_view const& discount,
...
{
  static constexpr char const* cuda = R"***(
    void
    transform(int64_t i,
              double * __restrict__ amount,
              double const* __restrict__ discount,
              double const* __restrict__ extended_price,
              double const* __restrict__ supply_cost,
              int8_t const* __restrict__ quantity)
  {
    amount[i] = extended_price[i] * (1 - discount[i]) - supply_cost[i] * quantity[i];
  }
  )***";
...
void ndsh_q9_transform(nvbench::state& state)
{
  ...
  state.exec(nvbench::exec_tag::sync, [&](nvbench::launch& launch) {
    run_ndsh_q9(
      state, sources, launch.get_stream().get_stream(), cudf::get_current_device_resource_ref());
  });
}

NVBENCH_BENCH(ndsh_q9_transform)
  .set_name("ndsh_q9_transform")
